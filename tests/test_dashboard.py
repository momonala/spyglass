...
from spyglass.dashboard.config import ChartDefinition, ChartSeries, MetricSelector, StateTransitionRule
...
def _point(name: str, metric_type: str, value: float, ts: str = "2026-05-27T10:00:00Z", tags: dict | None = None) -> dict:
...
@pytest.mark.parametrize("unit,expected", [("hours", "hours"), ("days", "days"), ("weeks", "weeks"), ("bad", "hours"), (None, "hours")])
...
def test_prepare_logs_parses_spyglass_format():
    logs = [{"timestamp": "2026-05-27T10:00:00Z", "level": "INFO", "logger_name": "app", "message": "2026-05-27 10:00:00,123 INFO [my_func] app.module hello world"}]
...
def test_prepare_logs_plain_message():
    logs = [{"timestamp": "2026-05-27T10:00:00Z", "level": "WARNING", "logger_name": "app", "message": "plain text"}]
    result = prepare_logs(logs)
